                                        ...
                                        GParamSpec *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_CONTENT_DATES:
      g_value_set_variant (value, priv->content_dates);
      break;
                                        ...
                                        GParamSpec *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_CONTENT_DATES:
      g_clear_pointer (&priv->content_dates, g_variant_unref);
      priv->content_dates = g_value_dup_variant (value);
      break;
...
eknc_article_object_model_class_init (EkncArticleObjectModelClass *klass)
{
  ...
  eknc_article_object_model_props[PROP_AUTHORS] =
    g_param_spec_variant ("authors", "Authors",
      ...
      G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS);
  /**
   * EkncArticleObjectModel:content-dates:
   *
   * A list of dates that the article being read refers to. The
   * dates are all in ISO8601.
   */
  eknc_article_object_model_props[PROP_AUTHORS] =
    g_param_spec_variant ("content-dates", "Content Dates",
      "A list of datest that the article being read refers to",
      G_VARIANT_TYPE ("as"), NULL,
      G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS);
  /**
                                              ...
                                              GArray *params)
{
  ...
  eknc_utils_append_gparam_from_json_node (json_object_get_member (object, "authors"),
                                           ...
                                           params);
  eknc_utils_append_gparam_from_json_node (json_object_get_member (object, "content_dates"),
                                           g_object_class_find_property (klass, "content-dates"),
                                           params);
